  ...
  if (wallet && enrichedLPs.length > 0) {
    try {
      ...
      for (const entry of entries) {
        ...
        // Staked positions auto-compound fees into the LP, so feesEarnedUsd
        // is already inside lp.usdValue — adding it here would double-count.
    ...
    try {
      ...
      if (oldestByPosition.size > 0) {
        ...
        for (const lp of enrichedLPs) {
          ...
          if (!old) continue;
          // For staked positions, fees are auto-compounded into principal —
          // counting them as a separate "24h earn" overlaps with the position
          // value increase. Exclude staked fees from both sides of the delta.
          const lpStaked = lp.protocol.includes("staked");
